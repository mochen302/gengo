...
import (
	...
	"github.com/mochen302/gengox/examples/set-gen/sets"
...
type CustomArgs struct {
	...
	FieldSkipTypes []string // choose which field Type should by skip
	FieldSkipNames []string // choose which field Name should by skip
}
...
func Packages(context *generator.Context, arguments *args.GeneratorArgs) generator.Packages {
	...
	boundingDirs := []string{}
	fieldSkipTypes := []string{}
	fieldSkipNames := []string{}
	if customArgs, ok := arguments.CustomArgs.(*CustomArgs); ok {
		if customArgs.BoundingDirs == nil {
			customArgs.BoundingDirs = context.Inputs
		}

		if len(customArgs.FieldSkipTypes) > 0 {
			fieldSkipTypes = customArgs.FieldSkipTypes
		}

		if len(customArgs.FieldSkipNames) > 0 {
			fieldSkipNames = customArgs.FieldSkipNames
		}
				...
				&generator.DefaultPackage{
					...
					GeneratorFunc: func(c *generator.Context) (generators []generator.Generator) {
						return []generator.Generator{
							NewGenDeepCopy(arguments.OutputFileBaseName, pkg.Path, boundingDirs, (ptagValue == tagValuePackage), ptagRegister, fieldSkipTypes, fieldSkipNames),
						}
					},
...
type genDeepCopy struct {
	...
	fieldSkipTypes map[string]struct{}
	fieldSkipNames map[string]struct{}
}

func NewGenDeepCopy(sanitizedName, targetPackage string, boundingDirs []string, allTypes, registerTypes bool, skipTypes, fieldSkipNames []string) generator.Generator {
	g := &genDeepCopy{
		...
		fieldSkipTypes: make(map[string]struct{}),
		fieldSkipNames: make(map[string]struct{}),
	}
	for _, v := range skipTypes {
		g.fieldSkipTypes[v] = struct{}{}
	}
	for _, v := range fieldSkipNames {
		g.fieldSkipNames[v] = struct{}{}
	}
	return g
}
...
func (g *genDeepCopy) doStruct(t *types.Type, sw *generator.SnippetWriter) {
	...
	for _, m := range ut.Members {
		if _, ok := g.fieldSkipNames[m.Name]; ok {
			klog.Infof("skip type name:%v package:%v by name", m.Name)
			continue
		}

		ft := m.Type
		if g.needFilterField(ft) {
			continue
		}
...
func (g *genDeepCopy) needFilterField(ft *types.Type) bool {
	if ft == nil {
		return false
	}

	typName := ft.Name.Package + "." + ft.Name.Name
	if _, ok := g.fieldSkipTypes[typName]; ok {
		klog.Infof("skip type name:%v package:%v by package.Name", ft.Name.Name, ft.Name.Package)
		return true
	}

	if _, ok := g.fieldSkipTypes[ft.Name.Package]; ok {
		klog.Infof("skip type name:%v package:%v by package", ft.Name.Name, ft.Name.Package)
		return true
	}
